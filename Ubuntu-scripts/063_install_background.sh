...
# Detect if this is an XFCE build
if dpkg -l | grep -q "^ii.*xfce4[^-]" || command -v xfce4-settings >/dev/null 2>&1; then
    echo "Detected XFCE desktop environment"
    IS_XFCE=true
else
    echo "Detected regular Ubuntu/GNOME desktop environment"
    IS_XFCE=false
fi
